** This do-file is designed for estimation with basic difference-in-differences model.

log using "${log}\6_other_results.smcl", replace
clear all
...
reg Employment DiD treat post
est sto Employment_0
reg Employment DiD treat post $plcae 
est sto Employment_1
reg Employment DiD treat post $plcae $xlist 
est sto Employment_2

reg LPR DiD treat post
est sto LPR_0
reg LPR DiD treat post $plcae 
est sto LPR_1
reg LPR DiD treat post $plcae $xlist 
est sto LPR_2	

esttab Employment_0 Employment_1 Employment_2 LPR_0 LPR_1 LPR_2 using "${table}\reg_basic.tex", keep(DiD treat post) se tex replace star(* .10 ** .05 *** .01)
